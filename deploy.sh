...
if [ -z "$cid" ]; then
  ...
  if sudo docker ps --format '{{.Names}}' | grep -Eq "^podotegreen\$"; then
    ...
    sudo docker-compose -f docker-compose.green.yml stop podotegreen
    echo 'deploy done! ✅'
    sudo docker ps -a
  else
    ...
    sudo docker-compose -f docker-compose.blue.yml stop podoteblue
    echo 'deploy done! ✅'
    sudo docker ps -a
  fi
fi 